...
import {
  ...
  promptApiMapping,
  getFilteredPrompts,
} from "./prompts/cisco-prompts.js";
import {
  ElicitationSchemas,
  createElicitationRequest,
} from "./elicitation/schemas.js";
import {
  shouldUseToonFormat,
  bugResponseToToon,
  caseResponseToToon,
  ...
  if (useToon) {
    try {
      if (apiName === "Bug" || toolName.includes("bug")) {
        return await bugResponseToToon(result as BugApiResponse);
      } else if (apiName === "Case" || toolName.includes("case")) {
        return await caseResponseToToon(result as CaseApiResponse);
      } else if (apiName === "EoX" || toolName.includes("eox")) {
        return await eoxResponseToToon(result as EoxApiResponse);
      } else {
        // Fallback to TOON for generic API responses
        return await bugResponseToToon(result as BugApiResponse);
      }
    } catch (error) {
      logger.warn("TOON formatting failed, falling back to standard format", {
        error,
      });
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      logger.info("Tool call started", {
        name,
        args,
        hasProgressToken: !!meta?.progressToken,
      });

      const { result, apiName } = await apiRegistry.executeTool(
        name,
        args || {},
        meta,
      );
      ...
      logger.info("Tool call completed", {
        ...
        apiName,
        resultCount:
          "bugs" in result && Array.isArray(result.bugs)
            ? result.bugs.length
            : "cases" in result && Array.isArray(result.cases)
              ? result.cases.length
              : "EOXRecord" in result && Array.isArray(result.EOXRecord)
                ? result.EOXRecord.length
                : 0,
      });

      const content: TextContent = {
        type: "text",
        text: await formatResults(result, apiName, name, args || {}),
      };
      ...
      resourceTemplates.push(
        {
          ...
          description:
            "Get product details by product ID (e.g., C9300-24P, ISR4431)",
          mimeType: "application/json",
        },
        {
          ...
          description:
            "Search Cisco product catalog by name or model (requires CISCO_WEB_COOKIE)",
          mimeType: "application/json",
        },
      );
    }
      ...
      resourceTemplates.push(
        {
          ...
          description:
            "Get security advisory by ID (e.g., cisco-sa-20180221-ucdm)",
          mimeType: "application/json",
        },
        {
          ...
          description:
            "Get security advisory by CVE identifier (e.g., CVE-2018-0101)",
          mimeType: "application/json",
        },
      );
    }
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    try {
      ...
      if (uri === "cisco://bugs/recent/critical") {
        ...
        description =
          "Recent high-severity bugs (severity 1-3) from last 30 days";
        const bugs1 = await apiRegistry.executeTool("search_bugs_by_keyword", {
          keyword: "bug",
          severity: "1",
          modified_date: "2",
...
`;
        ...
        if (!webCookie) {
          result = {
            ...
            message:
              "To use product autocomplete, you must provide your Cisco.com session cookie",
          ...
          try {
            ...
            if (response.status === 401 || response.status === 403) {
              result = {
                ...
                message:
                  "Your Cisco.com session cookie has expired or is invalid",
        ...
        if (productId === "catalog") {
          result = {
            description:
              "Use cisco://products/{product_id} to access specific product information",
            examples: [
          ...
          const productResult = await apiRegistry.executeTool(
        ...
        const advisories = await apiRegistry.executeTool(
        ...
        const advisory = await apiRegistry.executeTool(
          "get_security_advisory_by_id",
          {
